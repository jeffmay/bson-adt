...
import adt.bson.{BsonObjectWrites, BsonReads, BsonObject}
import reactivemongo.api.collections.default.BSONGenericHandlers.{BSONStructureWriter, BSONStructureReader}
import reactivemongo.api._
import reactivemongo.api.collections._
import reactivemongo.api.collections.default._
import reactivemongo.bson.buffer.ReadableBuffer
import reactivemongo.bson.{BSONObjectID, BSONDocumentWriter, BSONDocumentReader, BSONDocument}
 ...
 * }}}
 */
case class BsonValueCollection(
  db: DB,
  name: String,
  failoverStrategy: FailoverStrategy
) extends GenericCollection[BsonObject, BsonReads, BsonObjectWrites]
  with BSONGenericHandlers
...
//    BSONQueryBuilder(this, failoverStrategy)
  ...
  override def genericQueryBuilder: GenericQueryBuilder[BsonObject, BsonReads, BsonObjectWrites] = ???

  override def StructureBufferReader: BufferReader[BsonObject] = new BufferReader[BsonObject] {
    override def read(buffer: ReadableBuffer): BsonObject = buffer
  }

  override def StructureBufferWriter: BufferWriter[BsonObject] = ???

  override def StructureReader[T](reader: BsonReads[T]): GenericReader[BsonObject, T] = ???

  override def StructureWriter[T](writer: BsonObjectWrites[T]): GenericWriter[T, BsonObject] = ???
