...
import adt.bson.mongo.client.model.{IndexModel, WriteModel}
import adt.bson.mongo.client.result._
import adt.bson.{BsonJavaScript, BsonObject, BsonValue}
...
class BsonAdtAsyncCollection(underlying: MongoCollection[BsonObject]) {
  ...
  def listIndexes(): ListIndexesIterable[BsonValue] =
    new ListIndexesIterable(underlying.listIndexes(classOf[BsonValue]))
